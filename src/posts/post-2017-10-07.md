...
In the case of lingering decisions I have the feeling that sometimes we know what we want but we postpone it until we find the right story for us to fit it in our mental model, in a way we want to be able to explain why, it's not necessary that the explanation is right, but we need that to move forward.
